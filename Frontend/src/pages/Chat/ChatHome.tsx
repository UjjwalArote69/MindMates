  ...
  useEffect(() => {
    ...
    const initializeAndLoadChats = async () => {
      try {
        ...
        const handleAllChats = ({ chats }: { chats: ChatPreview[] }) => {
          if (mounted) {
            setChats(chats);
            setLoading(false);
          }
        ...
        // Attach listeners
        socketService.on("chat:all-chats", handleAllChats);
        socketService.on("chat:error", handleError);

        // Now emit the request
        socketService.emit("chat:get-all", {});

        // Cleanup function
        return () => {
          socketService.off("chat:all-chats", handleAllChats);
      ...
      <div className="bg-gradient-to-r from-[#4A7C59] to-[#5B9368] px-6 py-6 rounded-b-3xl shadow-lg">
        ...
        <div className="flex gap-6 text-white/90 text-sm">
          <div className="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-full backdrop-blur-sm">
            ...
            <span className="font-medium">{chats.length} Total</span>
          </div>
          <div className="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-full backdrop-blur-sm">
            <span className="text-lg">📅</span>
        ...
        {chats.length > 0 ? (
          ...
          <div className="text-center py-16 px-6">
            ...
            <p className="text-gray-500 text-sm mb-6">
              Start a new conversation with Dr. Mate
            </p>
            <button
