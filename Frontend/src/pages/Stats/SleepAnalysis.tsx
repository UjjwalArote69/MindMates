...
const SleepAnalysis = () => {
  ...
  const sleepLevels = [
    { level: 1, label: "Insomniac", color: "from-red-400 to-red-600", desc: "< 4 hours", emoji: "😴", value: 20 },
    { level: 2, label: "Light Sleeper", color: "from-orange-400 to-orange-600", desc: "4-5 hours", emoji: "😪", value: 40 },
    { level: 3, label: "Moderate", color: "from-yellow-400 to-yellow-600", desc: "5-6 hours", emoji: "😐", value: 60 },
    { level: 4, label: "Good Sleeper", color: "from-blue-400 to-blue-600", desc: "6-7 hours", emoji: "😊", value: 80 },
    { level: 5, label: "Sleep Master", color: "from-purple-400 to-purple-600", desc: "7-9 hours", emoji: "😌", value: 100 },
  ];

  // Calculate sleep level from hours
  const getSleepLevel = (hours: number) => {
    if (hours < 4) return 0;
    if (hours < 5) return 1;
    if (hours < 6) return 2;
    if (hours < 7) return 3;
  ...
  const sleepStats = useMemo(() => {
    ...
    user.sleepLogs.forEach(log => {
      ...
      if (dayIndex !== -1) {
        last7Days[dayIndex].hours = log.hours;
        last7Days[dayIndex].quality = log.quality;
        last7Days[dayIndex].level = getSleepLevel(log.hours);
      }
    });
  ...
  const weeklyAverage = useMemo(() => {
    const dataWithValues = sleepStats.filter(s => s.hours !== null);
    if (dataWithValues.length === 0) return 0;
    return (dataWithValues.reduce((sum, stat) => sum + stat.hours!, 0) / dataWithValues.length).toFixed(1);
  }, [sleepStats]);

  const averageQuality = useMemo(() => {
    const dataWithValues = sleepStats.filter(s => s.quality !== null);
    if (dataWithValues.length === 0) return 0;
    return Math.round(dataWithValues.reduce((sum, stat) => sum + stat.quality!, 0) / dataWithValues.length);
  }, [sleepStats]);

  const currentLevel = useMemo(() => {
    const avg = parseFloat(String(weeklyAverage));
    return getSleepLevel(avg);
  }, [weeklyAverage]);
  ...
  const trend = useMemo(() => {
    ...
    const recent = recentData.reduce((sum, s) => sum + s.hours!, 0) / recentData.length;
    const earlier = earlierData.reduce((sum, s) => sum + s.hours!, 0) / earlierData.length;
  ...
  const latestSleep = useMemo(() => {
    const withData = sleepStats.filter(s => s.hours !== null);
  ...
  return (
    <div className="relative min-h-screen w-full bg-gradient-to-br from-[#EFECE8] to-[#e5e0dc] overflow-hidden">
      {/* Decorative Shapes */}
      <div className="fixed top-0 right-0 w-96 h-96 bg-purple-200/20 rounded-full blur-3xl -mr-32 -mt-32 pointer-events-none animate-blob"></div>
      <div className="fixed bottom-0 left-0 w-96 h-96 bg-indigo-200/20 rounded-full blur-3xl -ml-32 -mb-32 pointer-events-none animate-blob-slow"></div>

      <div className="relative z-10 flex-1 overflow-y-auto pb-24">
        {/* Header */}
        <div className={`relative w-full bg-gradient-to-br ${sleepLevels[currentLevel].color} rounded-b-[40px] shadow-2xl transition-all duration-500 overflow-hidden`}>
          ...
          <div className="relative z-10 px-6 pt-8 pb-12">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-white/90 text-sm mb-1">Track your sleep quality</p>
              <p className="text-white/70 text-xs">Get insights for better rest</p>
            </div>
            ...
            <div className="flex flex-col items-center mb-8">
              <div className="relative mb-4">
                ...
                <div className="relative w-32 h-32 bg-white/30 backdrop-blur-md rounded-full flex items-center justify-center shadow-2xl">
                  <span className="text-7xl animate-bounce-slow">{sleepLevels[currentLevel].emoji}</span>
                </div>
              </div>
              
              <div className="px-6 py-2 bg-white/30 backdrop-blur-sm rounded-full shadow-lg mb-2">
                <span className="text-white font-bold text-lg">
                  {sleepLevels[currentLevel].label}
                </span>
              </div>
              
              <p className="text-white/80 text-sm">
                {weeklyAverage}h average sleep
              </p>
            </div>
              ...
              {sleepLevels.map((_, i) => (
                <div
                  ...
                  className={`h-2 rounded-full transition-all duration-300 ${
                    i <= currentLevel ? "w-10 bg-white" : "w-6 bg-white/40"
                  }`}
                />
              ))}
            </div>
        ...
        <div className="mx-6 -mt-8 relative z-20 grid grid-cols-3 gap-3 mb-6">
          <div className="bg-white/90 backdrop-blur-xl rounded-2xl p-4 shadow-lg text-center">
            ...
            <p className="text-2xl font-bold text-[#3C2C1D]">{weeklyAverage}h</p>
            <p className="text-xs text-gray-600 mt-1">Weekly Avg</p>
          </div>

          <div className="bg-white/90 backdrop-blur-xl rounded-2xl p-4 shadow-lg text-center">
            <Moon size={18} className="text-indigo-600 mx-auto mb-2" />
            <p className="text-2xl font-bold text-[#3C2C1D]">{sleepStats.filter(s => s.hours !== null).length}</p>
            <p className="text-xs text-gray-600 mt-1">Logs</p>
          </div>

          <div className="bg-white/90 backdrop-blur-xl rounded-2xl p-4 shadow-lg text-center">
            <Sparkles size={18} className="text-purple-600 mx-auto mb-2" />
            <p className="text-2xl font-bold text-[#3C2C1D]">{averageQuality}%</p>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div>
                      <p className="text-xs text-gray-600">Bedtime</p>
                      <p className="text-lg font-bold text-[#3C2C1D]">23:00</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div>
                      <p className="text-xs text-gray-600">Wake Up</p>
                      <p className="text-lg font-bold text-[#3C2C1D]">
                        {latestSleep.hours! < 12 ? `0${Math.floor(latestSleep.hours! + 23)}` : Math.floor(latestSleep.hours!)}:00
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold text-[#3C2C1D]">{latestSleep.hours}h</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-gray-600">Quality</p>
                    <p className="text-lg font-bold text-purple-600">{latestSleep.quality}%</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white/90 backdrop-blur-xl rounded-3xl p-6 shadow-2xl border border-white/20">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xl font-bold text-[#3C2C1D]">Weekly Overview</h2>
              </div>
                ...
                {sleepStats.map((stat, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="relative w-full">
                        {hasData ? (
                          <div
                            ...
                            style={{
                              height: `${(stat.hours! / 9) * 180}px`,
                              background: `linear-gradient(to top, ${sleepLevels[stat.level!].color.replace('from-', '').replace(' to-', ', ')})`,
                            }}
                          >
                      ...
                      <div className={`text-center ${isToday ? "text-indigo-600 font-bold" : "text-gray-600"}`}>
          ...
          <div className="bg-gradient-to-br from-blue-100 to-purple-100 rounded-3xl p-6 shadow-lg">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-bold text-[#3C2C1D] mb-2">Sleep Insight</h3>
                <p className="text-sm text-gray-700 leading-relaxed">
                  {currentLevel >= 4 ? (
                    <>You're getting <span className="font-bold text-green-600">excellent sleep</span>! Keep maintaining this routine. 🌟</>
                  ) : currentLevel === 3 ? (
                    <>Your sleep is <span className="font-bold text-yellow-600">moderate</span>. Try adding 30 minutes for optimal rest. 😊</>
                  ) : currentLevel === 2 ? (
                    <>You're getting <span className="font-bold text-orange-600">limited sleep</span>. Aim for 7-9 hours for better health. 💤</>
                  ) : (
                    <>You're experiencing <span className="font-bold text-red-600">sleep deprivation</span>. Please prioritize rest immediately. 🚨</>
                  )}
                </p>
                {trend.direction === "up" && (
                  <p className="text-xs text-green-600 mt-3 font-medium flex items-center gap-1">
                    <CheckCircle2 size={14} />
                    Great! Your sleep increased by {trend.value}h this week!
                  </p>
                )}
                {trend.direction === "down" && (
                  <p className="text-xs text-red-600 mt-3 font-medium flex items-center gap-1">
                    <AlertCircle size={14} />
                    Your sleep decreased by {trend.value}h. Try going to bed earlier.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              {sleepLevels.map((sleep, idx) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className={`font-bold text-sm ${currentLevel === idx ? "text-white" : "text-[#3C2C1D]"}`}>
                      {sleep.label}
                    </p>
                    <p className={`text-xs ${currentLevel === idx ? "text-white/80" : "text-gray-600"}`}>
                      {sleep.desc}
                    </p>
                  </div>
                  {currentLevel === idx && (
