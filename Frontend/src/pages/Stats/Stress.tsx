...
const Stress = () => {
  ...
  // ✅ FIXED: Proper date mapping with correct week display
  ...
  const weeklyAverage = useMemo(() => {
    ...
    return Math.round(
      dataWithValues.reduce((sum, stat) => sum + stat.value, 0) / dataWithValues.length
    );
  }, [stressStats]);
  ...
  const bestDay = useMemo(() => {
    ...
    return dataWithValues.reduce((min, stat) => 
  ...
  const worstDay = useMemo(() => {
    ...
    return dataWithValues.reduce((max, stat) => 
  ...
  return (
    <div className="relative w-full min-h-screen bg-gradient-to-br from-[#f8f5f2] to-[#f0e9e4] overflow-hidden">
      ...
      <div className="relative z-10 flex-1 overflow-y-auto pb-32">
        {/* Header */}
        <div
          className={`relative w-full bg-gradient-to-br ${stressLevels[currentStress].gradient} rounded-b-3xl shadow-2xl transition-all duration-500 overflow-hidden`}
        >
          {/* Decorative elements */}
          ...
          <div className="relative z-10 px-6 pt-8 pb-12">
            ...
            <div className="flex flex-col items-center mb-8">
              <div className="relative mb-4">
                ...
                <div className="relative w-32 h-32 bg-white/30 backdrop-blur-md rounded-full flex items-center justify-center shadow-2xl">
                  <span className="text-7xl animate-bounce-slow">{stressLevels[currentStress].emoji}</span>
                </div>
              </div>
              
              <div className="px-6 py-2 bg-white/30 backdrop-blur-sm rounded-full shadow-lg mb-2">
                <span className="text-white font-bold text-lg">
                  {stressLevels[currentStress].label}
                </span>
              </div>
              
              <p className="text-white/80 text-sm">
                {stressLevels[currentStress].description}
              </p>
            </div>
            ...
            <div className="flex items-center justify-center gap-2 px-4">
              {stressLevels.map((stress, idx) => (
                ...
                >
                  <span className={`text-2xl ${currentStress === idx ? "" : "opacity-70"}`}>
                    {stress.emoji}
                  </span>
        ...
        <div className="mx-6 mt-6">
          <div className={`bg-gradient-to-br ${stressLevels[currentStress].bgColor} rounded-3xl p-6 shadow-lg border-2 ${stressLevels[currentStress].bgColor} border-opacity-30`}>
            <div className="flex items-start gap-3">
              <div className={`w-12 h-12 bg-gradient-to-br ${stressLevels[currentStress].gradient} rounded-full flex items-center justify-center shadow-md flex-shrink-0`}>
                <Brain size={24} className="text-white" />
              </div>
              <div className="flex-1">
                <h3 className="text-lg font-bold text-[#4B2E2B] mb-2 flex items-center gap-2">
                  AI Insight
                  <Sparkles size={16} className={stressLevels[currentStress].textColor} />
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">
                  {currentStress <= 1 ? (
                    <>You're doing <span className="font-bold text-green-600">amazing</span>! Your stress levels are well managed. Keep up your healthy habits! 🌟</>
                  ) : currentStress === 2 ? (
                    <>Your stress is <span className="font-bold text-yellow-600">mild</span>. Perfect time to practice preventive techniques like meditation. 🧘</>
                  ) : currentStress === 3 ? (
                    <>Moderate stress detected. Try <span className="font-bold text-orange-600">deep breathing exercises</span> or a 10-minute walk. 🚶</>
                  ) : currentStress === 4 ? (
                    <>High stress levels. Please <span className="font-bold text-red-600">take a break</span> and practice relaxation techniques. 💙</>
                  ) : (
                    <>Very high stress detected. <span className="font-bold text-red-700">Reach out for support</span> - talk to someone you trust or a professional. 🆘</>
                  )}
                </p>
                {trend.direction === "up" && (
      ...
      {hasChanged && (
        <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-[#f8f5f2] via-[#f8f5f2] to-transparent pointer-events-none z-30">
          <button
            ...
            disabled={saving}
            className={`w-full max-w-md mx-auto flex items-center justify-center gap-3 py-4 bg-gradient-to-r ${stressLevels[currentStress].gradient} text-white text-lg font-bold rounded-2xl shadow-2xl hover:shadow-3xl disabled:opacity-50 active:scale-98 transition-all pointer-events-auto`}
          >
            {saving ? (
              <>
